...
namespace ASICPhaseCalculator
{
    ...
    void Canvas::update()
    {
        ...
        {   
            processor->setVisContChan(newId - 1);
    ...
    void Canvas::refresh(){
        ...
        CalVisualizerThread* calVisThreadPtr = processor->getCalVisualizerThreadPtr();

        // get new angles from visualization phase buffer
        if (calVisThreadPtr->tryToReadVisPhasesfromCompBuffer(tempPhaseBuffer)){
    ...
    void Canvas::comboBoxChanged(ComboBox* comboBoxThatHasChanged)
    {
        if (comboBoxThatHasChanged == cChannelBox)
        {
            ...
            processor->setVisContChan(int(newValue));
        }
        else if (comboBoxThatHasChanged == eChannelBox)
        {
            ...
            processor->setVisEventChan(int(newValue));
        }
    }
    ...
    */
    ...
    void RosePlot::reorganizeAngleData(){
        ...
        if (angleData == nullptr) {    
            newAngleData = new AngleDataMultiset(numBins, referenceAngle);  // construct empty container
        }
        else
        {
